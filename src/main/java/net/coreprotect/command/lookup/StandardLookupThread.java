...
import net.coreprotect.data.lookup.result.InventoryLookupResult;
        ...
        try (Connection connection = Database.getConnection(true)) {
            ...
            try (Statement statement = connection.createStatement()) {
                ...
                switch (lookupResult) {
                    ...
                    case InventoryLookupResult inventoryResult -> {
                        for (String[] data : lookupList) {
                            ...
                            String timeago = ChatUtils.getTimeSince(Integer.parseInt(time), currentUnixSeconds, true);
                    ...
                    case CommonLookupResult commonResult -> {
                        for (String[] data : lookupList) {
                            ...
                            String timeago = ChatUtils.getTimeSince(Integer.parseInt(time), currentUnixSeconds, true);
                            int timeLength = 50 + (ChatUtils.getTimeSince(Integer.parseInt(time), currentUnixSeconds, false).replaceAll("[^0-9]", "").length() * 6);
                            String leftPadding = Color.BOLD + Strings.padStart("", 10, ' ');
                            if (timeLength % 4 == 0) {
                                leftPadding = Strings.padStart("", timeLength / 4, ' ');
                            } else {
